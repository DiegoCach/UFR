...
public class patron : MonoBehaviour {
    ...
    private bool IrHaciaHito(Vector3 PosicionHito, float Velocidad)
    {
        ...
        if (Mathf.Abs(VectorHaciaObjetivo.x) > 0.2F ||
            Mathf.Abs(VectorHaciaObjetivo.y) > 0.2F ||
            Mathf.Abs(VectorHaciaObjetivo.z) > 0.2F)
        {
			...
			thisTransform.rotation = Quaternion.LookRotation(VectorHaciaObjetivo);
